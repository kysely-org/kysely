...
export default (_context: LoadContext): Plugin => ({
  ...
  async loadContent() {
    ...
    await rm(EXAMPLES_FOLDER_PATH, { force: true, recursive: true })
    ...
    await Promise.all(
      dirents.map(async (dirent) => {
        ...
        for await (const line of lineIterator) {
          ...
          if (!inChosenExample && CHOSEN_EXAMPLE_BLOCK_START_REGEX.test(line)) {
          ...
          console.log('writing example', filePath, state.annotation![0])
          await (recreateExamplesFolderPromise ||= mkdir(EXAMPLES_FOLDER_PATH, {
            recursive: true,
          }))
