...
class JsonReferenceBuilder<DB, TB extends keyof DB, O>
  implements Expression<O>
{
  readonly #column: string
  readonly #refs: string[]

  constructor(column: string, refs: string[]) {
    this.#column = column
    this.#refs = refs
  }
  ...
  col<SR extends AnyColumn<DB, TB> | AnyColumnWithTable<DB, TB>>(
    column: SR
  ): JsonReferenceBuilder<DB, TB, ExtractTypeFromStringReference<DB, TB, SR>> {
    return new JsonReferenceBuilder(column, [])
  }

  ref<R extends keyof O & string>(
    field: R
  ): JsonReferenceBuilder<DB, TB, O[R]> {
    return new JsonReferenceBuilder(this.#column, [...this.#refs, field])
  }

  as<A extends string>(alias: A): AliasedExpression<O, A> {
    return new AliasedJsonReferenceBuilder(this, alias)
  }

  toOperationNode(): OperationNode {
    return sql`${sql.ref(this.#column!)} -> ${sql.join(
      this.#refs,
      sql`->`
    )}`.toOperationNode()
  }
...
class AliasedJsonReferenceBuilder<O, A extends string>
  implements AliasedExpression<O, A>
{
  #expr: Expression<O>
  #alias: A

  constructor(expr: Expression<O>, alias: A) {
    this.#expr = expr
    this.#alias = alias
  }
...
function jsonRef<DB, TB extends keyof DB>(
  eb: ExpressionBuilder<DB, TB>
): JsonReferenceBuilder<DB, TB, unknown> {
  return new JsonReferenceBuilder('', [])
}
...
it.skip('wot', async () => {
  ...
  const query = db
    ...
    .select([
      'id',
      (eb) => jsonRef(eb).col('person.address').ref('postal_code').as('pc'),
    ])

  console.log(query.compile().sql)

  // const res: Promise<{ pc: number }> = query.executeTakeFirstOrThrow()
})
...
type Merge<I, S, U> = {
  [K in keyof I | keyof S | keyof U]: ColumnType<
...
type T = Merge<PersonInsert, PersonSelect, PersonUpdate>